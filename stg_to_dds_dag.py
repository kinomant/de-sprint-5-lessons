...
def ts_extract ():
    ...
    """)
    max_update_ts = PostgresHook(postgres_conn_id='PG_WAREHOUSE_CONNECTION').get_first("""select coalesce(max(workflow_settings), '1980-01-01')::timestamp(6) from dds.srv_wf_settings where workflow_key = 'timestamps'""")[0]
    for record in records:
        values = [[record[0], record[1], record[2], record[3], record[4], record[5]]]
        target_fields = ['ts', 'year', 'month', 'day', 'time', 'date']
        PostgresHook(postgres_conn_id='PG_WAREHOUSE_CONNECTION').insert_rows('dds.dm_timestamps', rows=values, target_fields=target_fields)
        if record[0] > max_update_ts:
            max_update_ts = record[0]
...
def orders_extract ():
    records = PostgresHook(postgres_conn_id='PG_WAREHOUSE_CONNECTION').get_records(
    """
    with tmp as (select object_id as order_key, 
    ...
    """)
    max_update_ts = PostgresHook(postgres_conn_id='PG_WAREHOUSE_CONNECTION').get_first("""select coalesce(max(workflow_settings), '1980-01-01')::timestamp(6) from dds.srv_wf_settings where workflow_key = 'orders'""")[0]
    for record in records:
        values = [[record[0], record[1], record[2], record[3], record[4]]]
        target_fields = ['user_id', 'restaurant_id', 'timestamp_id', 'order_key', 'order_status']
        PostgresHook(postgres_conn_id='PG_WAREHOUSE_CONNECTION').insert_rows('dds.dm_orders', rows=values, target_fields=target_fields)
        if record[5] > max_update_ts:
            max_update_ts = record[5]
            PostgresHook(postgres_conn_id='PG_WAREHOUSE_CONNECTION').insert_rows('dds.srv_wf_settings', rows=[['orders', max_update_ts]], commit_every=100, target_fields=target_fields_srv_wf_settings)
